...
SELECT SONG_TITLE,ALBUMS.ALBUM_TITLE 
...
SELECT ALBUM_TITLE,COUNT(SONGS.SONG_ID) 
FROM ALBUMS JOIN SONGS 
ON ALBUMS.ALBUM_ID=SONGS.ALBUM_ID
GROUP BY ALBUM_TITLE

--24 Retrieve all songs and release year and sort them by release year.
SELECT SONG_TITLE,ALBUMS.release_year 
FROM SONGS JOIN ALBUMS 
ON SONGS.ALBUM_ID=ALBUMS.ALBUM_ID
ORDER BY ALBUMS.RELEASE_YEAR

--25 Retrieve the total number of songs for each genre, showing genres that have more than 2 songs.
SELECT GENRE,COUNT(DISTINCT SONG_TITLE) 
FROM SONGS 
GROUP BY GENRE 
HAVING COUNT(DISTINCT SONG_TITLE)>2
